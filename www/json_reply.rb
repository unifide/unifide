...
class JSONReply
    ...
    def self.process_project(user, project_name, hm)
        project = Project.find_by_short_name project_name
	if !project.nil?
	    #The project exists
            if user_can_see_project? user, project
    ...
    def self.process_unit_type(user, project_name, type_name, hm)
    ...
    def self.process_unit(user, project_name, type_name, name, hm)
	self.process_unit_type user, project_name, type_name, hm do |project, unit_type|
    ...
    def self.get_unit(user, project_name, type_name, name, depth)
        hm = {:success => false}
	self.process_unit(user, project_name, type_name, name, hm) do |project, unit_type, unit|
	    #Let's make a hash!
	    hm[:success] = true
	    hm[:depth] = depth
    ...
    def self.create_unit(user, project, type, name)
    end
